...
from seldom_atx.utils.solox.common import Platform
collect_fps = 0
collect_jank = 0
...
class SurfaceStatsCollector(object):
    ...
    def start(self, start_time):
        if not self.use_legacy_method:
            try:
                ...
                logger.warning(
                    u'Unable to dynamically obtain the current activity name, using page_ Flip statistics full screen frame rate')
    ...
    def get_surfaceview(self):
        ...
        activity_line = ''
        try:
            dumpsys_result = adb.shell(cmd='dumpsys SurfaceFlinger --list | {} {}'.format(
                ('grep', 'findstr')[platform.system() == Platform.Windows], self.package_name), deviceId=self.device)
    ...
    def get_surfaceview_activity(self):
        ...
        try:
            dumpsys_result = adb.shell(
                cmd='dumpsys SurfaceFlinger --list | {} {}'.format(
                    ('grep', 'findstr')[platform.system() == Platform.Windows], self.package_name),
            ...
            if activity_line:
                if activity_line.find(' ') != -1:
                    ...
                    activity_name = activity_line.replace('SurfaceView', '').replace('[', '').replace(']', '').replace(
                        '-', '').strip()
            else:
                activity_name = dumpsys_result_list[len(dumpsys_result_list) - 1]
                if not activity_name.__contains__(self.package_name):
